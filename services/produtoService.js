...
const update = async (cliente) => {
  ...
  const isOk = await (await connection).execute(query, [cliente.nome, cliente.descricao, cliente.preco, cliente.data_atualizado, cliente.id]);
  return isOk[0].affectedRows === 1;
};

const save = async (cliente) => {
  const query = 'INSERT INTO produtos(nome, descricao, preco,data_atualizado) VALUES (?, ?, ?, ?)';
  const isOk = await (await connection).execute(query, [cliente.nome, cliente.descricao, cliente.preco, cliente.data_atualizado]);
  return isOk[0].affectedRows === 1;
};
